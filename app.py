      ...
      updatemenus=[go.layout.Updatemenu(
          ...
          buttons=list(
              [dict(label = 'Number of Cases',
                    method = 'update',
                    args = [{'visible': [x == 0 for x in mask]}, 
               ...
               dict(label = 'Case Hours',
                    method = 'update',
                    args = [{'visible': [x == 2 for x in mask]}, 
                            {'title': 'Case Hours',
                             'showlegend':True}]),
               ...
               dict(label = 'Billings',
                    method = 'update',
                    args = [{'visible': [x == 4 for x in mask]},
                            {'title': 'Billings',
                             'showlegend':True}])
              ])
          )
...
fig_pie.update_layout(title='Number of Cases',
    updatemenus=[go.layout.Updatemenu(
         ...
         buttons=list(
             [dict(label = 'Number of Cases',
                   method = 'update',
                   args = [{'visible': [x == 0 for x in pimask], 'texttemplate' : '%{value:.0f}'}, 
                           {'title': 'Number of Cases', 'showlegend':True}]),
               dict(label = 'Lifetime Value',
                    method = 'update',
                    args = [{'visible': [x == 1 for x in pimask], 'texttemplate' : '\u00A3%{value}'},
                            {'title': 'Lifetime Value', 'showlegend':True}]),
               dict(label = 'Case Hours',
                    method = 'update',
                    args = [{'visible': [x == 2 for x in pimask], 'texttemplate' : '%{value:.0f}'},
                            {'title': 'Case Hours', 'showlegend':True}]),
               dict(label = 'Case Duration',
                    method = 'update',
                    args = [{'visible': [x == 3 for x in pimask], 'texttemplate' : '%{value:.0f}'},
                            {'title': 'Case Duration', 'showlegend':True}]),
               dict(label = 'Billings',
                    method = 'update',
                    args = [{'visible': [x == 4 for x in pimask], 'texttemplate' : '%{value:.0f}'}, 
