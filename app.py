...
fig_pie.update_layout(title='Number of Cases',
    updatemenus=[go.layout.Updatemenu(
         ...
         buttons=list(
             [dict(label = 'Number of Cases',
                   method = 'update',
                   args = [{'visible': [x == 0 for x in pimask]}, 
                           {'title': 'Number of Cases', 'showlegend':True}]),
               dict(label = 'Lifetime Value',
                    method = 'update',
                    args = [{'visible': [x == 1 for x in pimask], 'texttemplate' : '%{value:£,s}'},
                            {'title': 'Lifetime Value', 'showlegend':True}]),
               dict(label = 'Case Hours',
                    method = 'update',
                    args = [{'visible': [x == 2 for x in pimask], 'texttemplate' : f'{int(value)} hours {round(((value % 1)*60)/5)*5} minutes'}, 
               ...
               dict(label = 'Billings',
                    method = 'update',
                    args = [{'visible': [x == 4 for x in pimask]}, 
